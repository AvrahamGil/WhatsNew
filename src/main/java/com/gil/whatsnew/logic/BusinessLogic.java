...
import com.gil.whatsnew.dao.BusinessDao;
...
public class BusinessLogic {
		...
		try {
			if(!articles.isEmpty()) {
				for(ContextApi article : articles) {
					businessDao.addArticle(article);
				}
			}
	...
	public ContextApi getRandomArticles() throws ApplicationException {
		...
		article = businessDao.getRandomArticle();
	...
	public List<ContextApi> getListOfAllNewsArticles() throws ApplicationException {
		...
		try {
			...
			articles = businessDao.getAllArticles();
	...
	public void deleteArticles() throws ApplicationException {
		try {
			businessDao.deleteArticle();
		}catch(ApplicationException e) {
			ExceptionHandler.generatedLogicExceptions(e);
		}
	}
	
	public List<ContextApi> getArticles(BusinessType businessType) throws ClientProtocolException, IOException {
		List<ContextApi> articles = new ArrayList<ContextApi>();
		HttpResponse response = request.getNewsArticleByType(businessType.getSite());
		List<ContextApi> tempArticles = generatedArticleList(response);
		String lastTitle = "";
		String lastUrl = "";
		
		for (ContextApi article : tempArticles) {
			if (article.getUrl().contains(businessType.getSite()) && counter <= maxCounter
				...
				article.setNewsType(businessType.toString());
	...
	public void addingTypesSites(List<BusinessType> recieveTypes) {
		recieveTypes.clear();

		recieveTypes.add(BusinessType.Zacks);
		recieveTypes.add(BusinessType.Bloomberg);
		recieveTypes.add(BusinessType.CleanTechnica);
		recieveTypes.add(BusinessType.CNBC);
		recieveTypes.add(BusinessType.Fool);
		recieveTypes.add(BusinessType.Forbes);
		recieveTypes.add(BusinessType.FT);
		recieveTypes.add(BusinessType.IBtimes);
		recieveTypes.add(BusinessType.BusinessInsider);
		recieveTypes.add(BusinessType.Entrepreneur);
		recieveTypes.add(BusinessType.YahooFinance);
		recieveTypes.add(BusinessType.MarketWatch);
