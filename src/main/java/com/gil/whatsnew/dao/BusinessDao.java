...
@Repository
@Transactional
public class BusinessDao implements IArticlesDao{
	...
	public void addArticle(ContextApi article) throws ApplicationException {
		try {
				...
				Query query = session.createSQLQuery("insert into business (Title , Description , Url , ImageUrl ,BusinessType) "
						+ "value (:Title, :Description, :Url, :ImageUrl,:BusinessType)");
				...
				query.setParameter("BusinessType", article.getNewsType());
				query.executeUpdate();
	...
	public List<ContextApi> getAllArticles() throws ApplicationException {
		try {
			session = getSessionFactory().getCurrentSession();
			Query query = session.createSQLQuery("select Title,Description,Url,ImageUrl,BusinessType from business");
	...
	public ContextApi getRandomArticle() throws ApplicationException {
		try {
			session = getSessionFactory().getCurrentSession();
			Query query = session.createSQLQuery("select Title,Description,Url,ImageUrl,BusinessType from business order by RAND() limit 1");
	...
	public void deleteArticle() throws ApplicationException {
		try {
			session = getSessionFactory().getCurrentSession();
			Query query = session.createSQLQuery("delete from business");
			query.executeUpdate();
